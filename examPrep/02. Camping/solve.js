class SummerCamp {
    ...
    unregisterParticipant(name) {
       ...
       // if (!this.listOfParticipants.some(x => x.name === name)) {
          //  return `The ${name} is not registered at the camp.`;
       // }
        // the name that's not equal to the x.name disappears because it's not in the listOfParticipants
      //  this.listOfParticipants = this.listOfParticipants.filter(x => x.name != name);

        return `The ${name} is removed successfully.`
    }
    ...
    timeToPlay(typeOfGame, participant1, participant2) {
        ...
        if (typeOfGame === 'Battleship') {
            ...
            if (player1.power > player2.power) {
                player1.wins++;
                return `The ${player1.name} is winner in the game of ${typeOfGame}`
            } else if (player2.power > player1.power) {
                player2.wins++;
                return `The ${player2.name} is winner in the game of ${typeOfGame}`
            } else {
